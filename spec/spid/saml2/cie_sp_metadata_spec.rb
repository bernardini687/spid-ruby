...
RSpec.describe Spid::Saml2::CieSPMetadata do
  ...
  let(:settings) do
    instance_double(
      ...
      sp_organization: { name: "name", display_name: "display_name", url: "url" },
      sp_contact_person: { public: true, ipa_code: "ipa_code", email: "email" },
  ...
  describe "#to_saml" do
    ...
    describe "md:EntityDescriptor node" do
      ...
      describe "md:Organization node" do
        let(:xpath) { "#{super()}/md:Organization" }
      ...
      describe "md:ContactPerson node" do
        let(:xpath) { "#{super()}/md:ContactPerson" }
